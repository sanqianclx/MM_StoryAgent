...
class DashScopeImageGenerator:
    ...
    def __init__(self, api_key: str, model: str = "wanx-v1"):
        ...
        self.styles = {
            '(No style)': '{prompt}',
            'Japanese Anime': 'anime artwork illustrating {prompt}. created by japanese anime studio. highly emotional. best quality, high resolution',
            'Digital/Oil Painting': '{prompt} . Extremely Detailed Oil Painting, glow effects, godrays, Hand drawn, render, 8k, octane render',
            'Pixar/Disney Character': 'Create a Disney Pixar 3D style illustration on {prompt} . The scene is vibrant, motivational, filled with vivid colors',
            'Photographic': 'cinematic photo {prompt} . Hyperrealistic, Hyperdetailed, detailed skin, matte skin, soft lighting, realistic, best quality',
            ...
            'Black and White Film Noir': '{prompt} . b&w, Monochromatic, Film Photography, film noir, analog style, soft lighting',
        ...
        self.negative_prompt = "naked, deformed, bad anatomy, disfigured, poorly drawn face, mutation, extra limb, ugly, disgusting, poorly drawn hands, missing limb"

    def apply_style(self, style_name: str, prompt: str):
        """应用风格模板"""
        style_template = self.styles.get(style_name, self.styles["(No style)"])
    ...
    def call(self, prompts: List[str], style_name: str = "Storybook", **kwargs):
        """批量生成图像"""
        images = []
        for i, prompt in enumerate(prompts):
            print(f"生成第 {i+1}/{len(prompts)} 张图像: {prompt[:50]}...")
            image = self.generate_image(prompt, style_name)
            images.append(image)
        return images
...
class StoryDiffusionAgent:
    ...
    def call(self, params: Dict):
        ...
        # 使用DashScope API生成图像
        generation_agent = DashScopeImageGenerator(
            api_key=self.cfg.get("api_key", os.getenv("DASHSCOPE_API_KEY")),
            model=self.cfg.get("model", "wanx-v1")
        )
